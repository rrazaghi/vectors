...
#      https://www.fctables.com/italy/serie-a/
    ...
    #session.get(url)
    dfs_epl = pd.read_html(session.get('https://www.fctables.com/england/premier-league/').text,header=0)
    table_epl = dfs_epl[0]
    table_epl.set_index('Team', inplace=True)
    table_epl_dict = table_epl.T.to_dict('list')
    
    dfs_sa = pd.read_html(session.get('https://www.fctables.com/italy/serie-a/').text,header=0)
    table_sa = dfs_sa[0]
...
#      https://www.fctables.com/italy/serie-a/
    ...
    leaderboard_df = leaderboard_df.sort_values(by=['PTS', 'GD'], ascending=False)
    leaderboard_df = leaderboard_df.astype(int)
    return render_template('index.html',table=leaderboard_df.to_html(classes='scores'))
